...
pp_1998_2012 <- read.csv("../data/pp_1998-2012.csv", stringsAsFactors = F)
```
...
df <- as.tbl(pp_1998_2012) %>%
  ...
  mutate(value = ifelse(wertung2=="Ablehn.", -1,0),
         ...
         medium = factor(ifelse(grepl("ddeutsche", medium2), "SZ", medium2)),
...
theme.df <- df %>%
  ...
  summarise(m_value = mean(value, na.rm = TRUE),
            obs = n(),
            total_obs = nrow(df),
            weight = obs/total_obs,
            w_value = m_value*weight)
```

```{r fig.height=14, fig.width=8}
ggplot(theme.df, aes(reorder(p_them2, w_value), w_value)) +
  geom_col(fill = col[1]) +
  labs(x="", y="weighted values") +
  coord_flip() 
```
